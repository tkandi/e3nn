...
class TensorProduct(torch.nn.Module):
    ...
    def tensor_product_left_right(self, x1s, x2s, weights):
        ...
        # We produce no code for empty self.instructions
        self.instructions = [ins for ins in self.instructions if 0 not in ins.path_shape]

        if len(self.instructions) == 0:
            outputs = x1s.new_zeros(output_shape + (self.irreps_out.dim,))
        ...
        weight_numel = sum(prod(ins.path_shape) for ins in self.instructions if ins.has_weight)
        if weight_numel > 0:
        ...
        for ins in self.instructions:
            ...
            if l1l2l3 not in self.w3j_dict:
                self.w3j_dict[l1l2l3] = o3.wigner_3j(mul_ir_in1.ir.l, mul_ir_in2.ir.l, mul_ir_out.ir.l, device="cuda")
            w3j = self.w3j_dict[l1l2l3]
    ...
    def forward(self, x, y, weight: Optional[torch.Tensor] = None):
        ...
        return self.tensor_product_left_right(x, y, real_weight)
